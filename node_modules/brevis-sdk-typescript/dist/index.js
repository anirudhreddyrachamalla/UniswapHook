...
var grpc_12 = __toESM(require("@grpc/grpc-js"));
var _one_of_decls21;
...
var Brevis = class {
  constructor(url) {
    const cred = import_grpc_js.credentials.createSsl();
...
function asUint248(input) {
  const big = import_ethers.ethers.getBigInt(input);
...
function asBytes32(input) {
  if (!import_ethers.ethers.isHexString(input)) {
    throw new Error(`value ${input} is not bytes32`);
  }
  const bs = import_ethers.ethers.getBytes(input);
  if (bs.length !== 32) {
...
var Prover = class {
  constructor(url) {
    const cred = import_grpc_js2.credentials.createInsecure();
...
function validateStorage(d) {
  ...
  if (!import_ethers2.ethers.isAddress(d.address)) err("storage.address", d.address);
...
function validLen(d) {
  if (!import_ethers2.ethers.isBytesLike(d)) {
    d = import_ethers2.ethers.toBeHex(d);
  }
  const len = import_ethers2.ethers.dataLength(d);
  return len > 0 && len <= 32;
}
